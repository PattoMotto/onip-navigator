...
const CalculatorForm: React.FC<Props> = ({ profile, onChange }) => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto pb-48">
      ...
      {/* SECTION 2: EDUCATION - Now exposed for Job Streams too (International Student etc) */}
      {(isGradStream || isEE || isJobStream) && (
      ...
      <InputSection title="Regionalization" active={isEOI}>
        <div className="space-y-4">
          ...
          {/* Study Location (Grad Streams OR Job Streams) */}
          {(isGradStream || isJobStream) && (
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                {isGradStream ? 'Location of Study (Campus)' : 'Location of Study (if applicable)'}
              </label>
              <select
                className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow hover:border-slate-300"
                value={profile.studyLocation || ''}
                onChange={(e) => onChange({ studyLocation: e.target.value })}
              >
                <option value="">Select Location...</option>
                {REGIONS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
              </select>
              {isJobStream && <p className="text-xs text-slate-500 mt-1">International students can claim points for studying in Ontario.</p>}
            </div>
          )}
        </div>
      </InputSection>
