...
export const calculateOINPScore = (profile: Profile): ScoreResult => {
    ...
    if ([StreamType.FOREIGN_WORKER, StreamType.INTL_STUDENT, StreamType.IN_DEMAND].includes(stream)) {
        ...
        breakdown['Regional Location (Job)'] = regionScore;
        total += regionScore;
        maxPossible += 10;
        ...
        // 4. Regionalization
        // Factor: Location of Study
        const studyRegionScore = getRegionScore(profile.studyLocation, stream);
        breakdown['Regional Location (Study)'] = studyRegionScore;
        total += studyRegionScore;
        maxPossible += 10;
